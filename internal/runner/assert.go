...
// appending any failures to tr.Failures. The apply-tier extras (snapshot
// store, live-cluster context) are threaded back in when those slices land.
//
// It does not set tr.Pass — callers decide that from len(tr.Failures) after
// any tier-specific pre-checks have also run.
func evaluateAssertions(tr *TestResult, et expandedTest, suite *dsl.Suite, allDocs []map[string]any, renderErr error) {
	test := et.Test

	evalContext := func(doc map[string]any) matchers.EvalContext {
	...
	if test.ForEach {
		...
		for docIdx, doc := range matchedDocs {
			for assertIdx, assertion := range test.Asserts {
				...
				result := matchers.Evaluate(assertion, evalContext(doc))
				if !result.Pass {
